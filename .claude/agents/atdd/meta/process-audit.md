...
1. **Discover and read.** `Glob` `docs/atdd/process/*.md` and `Read` each match in full. Discover the file list at runtime with `Glob` — do not hardcode it, so newly added process docs are picked up automatically. Do not summarise from headings alone.
