...
- [ ] **Step 3 — clauderun renders the attempt placeholders.** In `internal/atdd/process/clauderun/clauderun.go`, add `AttemptNumber`/`AttemptMax` to the `Options` struct (near line 276) and fill the `params` map (near line 770, alongside `scope-block`) with an **`${attempt-block}`** pre-rendered string — non-empty (e.g. "Attempt #2 of 2 — the loop halts after this pass.") when `AttemptMax > 0`, otherwise leave it unfilled. **Constraint:** `findUnfilledPlaceholders` (`internal/atdd/process/clauderun/clauderun.go:580,690`) hard-fails any dispatch whose prompt references an unfilled `${…}`. Use exactly one pre-rendered `${attempt-block}`, placed only in the fixer preamble (always a loop node, so always fillable) — do **not** expose raw `${attempt-number}`/`${attempt-max}` to prompt authors (Resolved decision 5). That keeps non-loop prompts that never mention it safe from the hard-fail.
...
4. **String state keys, not a typed accessor.** The engine writes `visit-count` / `visit-max` via `Context.Set` (string keys), not a typed `Context` accessor — lower engine-API surface, matches how other run state is threaded. (Resolves former Open question 1; the only execution-time check is the Step 1 grep for a reserved-key collision.)
5. **Single pre-rendered `${attempt-block}`, no raw placeholders.** Prompts get one pre-rendered `${attempt-block}` string only; raw `${attempt-number}`/`${attempt-max}` are **not** exposed to prompt authors. Keeps non-loop prompts safe from `findUnfilledPlaceholders`. (Resolves former Open question 2; only the final block wording is settled during Step 3/4 encoding.)
