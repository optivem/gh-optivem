...
### Step 1. Extend `projectconfig` schema

`internal/projectconfig/config.go`:

- Add `ProcessFlow string yaml:"process_flow,omitempty"`.
- Add `AgentPrompts map[string]string yaml:"agent_prompts,omitempty"`.
- Add `NodeExtras map[string]string yaml:"node_extras,omitempty"` —
  values are literal text appended to the named node's prompt.
- Add `NodeReplacements map[string]string yaml:"node_replacements,omitempty"` —
  values are paths whose file body replaces the named node's prompt.
- Validate that any `process_flow` / `agent_prompts` / `node_replacements`
  paths pass `validatePath` (repo-relative, no `..`).
- Validate that `agent_prompts` keys are members of `agents.Names()` —
  surface typos at config-load, not deep inside the pipeline.
- Validate that `node_extras` and `node_replacements` keys exist as node
  IDs in the resolved process-flow YAML. Two-phase: config-load checks
  shape/paths; the driver re-validates keys against the loaded process
  flow at startup (since process-flow is itself configurable, the node-ID
  set isn't known at config-load). Reject the same key appearing in both
  `node_extras` and `node_replacements` — replace strictly supersedes
  extras, so the combination signals operator confusion.

### Step 2. Delete `--system-config`, `--test-config`

`runner_commands.go`:

- Drop `systemPath` / `testsPath` flag vars from `newBuildSystemCmd`,
  `newRunSystemCmd`, `newStopSystemCmd`, `newCleanSystemCmd`, `newTestSystemCmd`.
- Replace `resolveSystemPath` / `resolveTestsPath` with simpler 2-tier
  helpers: YAML field → default. Or inline if it stays one-liner.
- Update `hintIfMissing` hint text: no longer mention the flag.
- Update `flagSystemUsage` / `flagTestsUsage` constants → delete.

Tests: `runner_commands_test.go` — strip flag-driven cases, keep YAML/default
cases.
...
- Drop `projectURL` flag var from `newAtddImplementTicketCmd`,
  `newAtddManageProjectCmd`, `newAtddDebugPickTopReadyCmd`.
- `driver.Options.ProjectURL` and `board.Options.ProjectURL` keep their
  fields (loaded from `gh-optivem.yaml` via `driver.loadDriverConfig`).
- Verify that `projectconfig.Validate` already rejects empty `project.url`
  (Rule 9 in `config.go:324`) — it does.

Tests: search for `--project` references; strip from atdd command tests.

### Step 4. Replace `--repo-dir` with `--workspace`; drop `$GH_OPTIVEM_WORKSPACE`

`internal/atdd/runtime/repolocator/repolocator.go`:

- Drop `EnvWorkspace` constant and `os.Getenv(EnvWorkspace)` lookup.
- Change `Resolve` signature:
  `Resolve(cfg *projectconfig.Config, workspace string, cwd string) (Result, error)`
- Body: `if workspace == "" { workspace = filepath.Dir(cwd) }` then
  `out.Local[slug] = filepath.Join(workspace, repoName(slug))` for every slug.
- Drop the mono-repo special case (folded into the general formula).
- Drop `ParseRepoDirFlag` entirely.

`atdd_commands.go`:

- Replace `--repo-dir` `StringSliceVar` with `--workspace` `StringVar`.
- Drop `repolocator.ParseRepoDirFlag` call; pass the workspace string directly.
- Update `runImplementTicketPreflight` signature.

`internal/atdd/runtime/preflight/...`:

- Update `preflight.Run` signature to take `workspace string` instead of
  `repoDirs map[string]string`.

Tests: `repolocator_test.go` — rewrite around the new formula.
