...
> Steps 1–6 completed 2026-05-14. The new package lives at `internal/atdd/runtime/tracker/`; the github adapter satisfies all seven Tracker methods. Consumers (`actions/bindings.go`, `gates/bindings.go`, `intake/parse.go`) route through `Tracker.PickReady` / `Tracker.SetStatus` / `Tracker.Classify` / `Tracker.ReadSections` / `Tracker.MarkChecklistComplete`. The `classify/` package is deleted. The shared markdown parser is **not yet** extracted to `tracker/internal/parse/` — github adapter inlines its own H2/H3 walker, and step 7 (markdown adapter) is expected to extract the shared parser at the same time it builds the second consumer. The old `internal/atdd/runtime/board/` package is still imported by `driver/driver.go` (FindIssue + ResolveProjectURLFromConfig) and `preflight_helpers.go` — step 13 deletes it once those switch over.

7. **Build markdown adapter.** Implement all seven `Tracker` methods in `tracker/markdown/`. See the adapter-map table above for behavior per method. Extract the shared markdown parser into `tracker/internal/parse/` and refactor `github/`'s inline H2/H3 walker + checkbox-rewrite to use it.
