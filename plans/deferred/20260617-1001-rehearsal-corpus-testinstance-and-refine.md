...
- [ ] 5c. Author a **story** ticket in `optivem/shop` (Gherkin AC, not Checklist) that reads data from the new external system — e.g. surface a customer's name on an order/confirmation by fetching it from the customer directory. The story must trip `at-external-driver-port-changed` so the contract-test excursion runs.
