...
> **Child of** `20260615-0548-gh-optivem-modular-monolith-parent.md`. This is the **last and hardest** module in the modular-monolith decomposition: the generic process engine and its pluggable process definition and agent set. All other children (#1–#6) are done; this one ran last by design because it depends on them. Its hard prerequisite — **seam #1 (`projectconfig → atdd/runtime/statemachine`)** — is now **resolved**: the lone engine-backed rule moved to `internal/atdd/runtime/configcheck`, so `internal/atdd/runtime/statemachine` is import-clean of the rest of `atdd` and ready to carve. (Scaffolding is already extracted as its own peer module at `internal/scaffolding/` — out of scope here.) Parent holds the cross-module vision, dependency rules, and the inventory that spawns the other children.
...
- [ ] Step 5: **Carve out the process engine** — move generic execution machinery into an engine package with no `atdd`-specific imports; the engine compiles and runs without any concrete process baked in. Its center of gravity, `internal/atdd/runtime/statemachine`, is **already import-clean** of the rest of `atdd` (seam #1 resolved — the engine-backed config rule lives in `internal/atdd/runtime/configcheck`). The shared **build** module (`internal/build/{runner,compiler}`) is **not** absorbed into the engine — it stays a separate dependency that both Process and Scaffolding point at (parent decision).
...
- **Existing coupling** — the parent's Step-1 inventory is now complete and shows the periphery is already extracted (kernel, build, config, diagrams, devworkflow, scaffolding) and `internal/atdd/runtime/statemachine` is already import-clean of the rest of `atdd` (seam #1 resolved — the engine-backed config rule now lives in `internal/atdd/runtime/configcheck`). Remaining entanglement is confined to the `atdd/runtime` internals (driver, agents, actions, gates, verify, etc.) — how tightly those bind to the generic state-machine core is the open question that affects effort estimate.
