...
**Step 9 — document the reuse path as package `doc.go` files.** Steps 7 and 8 are **DONE and committed**. Step 7: the `AgentSet` binding (`NewAgentSet`/`NewAgentSetFS`/`DefaultAgentSet`, five funcs as methods, injectable `fs.FS`); `clauderun.Options.AgentSet` + `driver.Options.AgentSet` thread it (nil → ATDD default). Step 8: both axes proven — `internal/engine/statemachine/reuse_process_test.go` (process axis: a non-ATDD `publish-doc` BPMN via `LoadBytes`, package `statemachine_test`) and `internal/atdd/runtime/driver/swappable_agentset_test.go` (agent axis: real `process-flow.yaml` rebound to an in-memory stub set via `NewAgentSetFS`). Now write two `doc.go` files: (a) `internal/engine/statemachine/doc.go` — the `LoadBytes` "bring your own BPMN" flow contract, citing `reuse_process_test.go` as the worked example; (b) the agent-set binding contract on the agents package (`internal/atdd/runtime/agents/doc.go`) — `NewAgentSetFS` "bring your own agents", citing `swappable_agentset_test.go`. Name the two contracts + the two swap points. Self-contained, no cross-language references. Stop at: `go build ./...` + `go test ./...` green (`go vet`/`go doc` render cleanly).

## Steps

- [ ] **Step 9: Document the reuse path.** Write the guide as **package `doc.go`** (decided) — the contract lives next to the code it describes and moves with it: `internal/engine/statemachine/doc.go` for the `LoadBytes` flow contract, and the agents package (`internal/atdd/runtime/agents/doc.go`) for the agent-set binding contract. Cover the two contracts and the two swap points, with the Step-8 fixtures (`reuse_process_test.go`, `swappable_agentset_test.go`) as worked examples. Keep it self-contained (no cross-language cross-references).
