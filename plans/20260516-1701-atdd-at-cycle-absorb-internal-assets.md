# Plan: bring `docs/atdd/process/at-cycle.md` to parity with internal/assets — Part 1: Cycle architecture & §Conventions

**Date:** 2026-05-16 (split into Part 1 / Part 2 / Legacy on 2026-05-18 during refinement)
**Context:** The goal is to eventually delete `internal/assets/`. `docs/atdd/process/at-cycle.md` is intended to become the canonical home for the AT cycle process spec, replacing the four global process pages under `internal/assets/global/docs/atdd/process/at-{red,green}-*.md`.
...
- **CT-cycle parity work** (sub-cycle of AT, invoked from AT-RED-EXTERNAL-SYSTEM-DRIVER): `docs/atdd/process/ct-cycle.md` likely has the same gaps vs its four internal CT pages (ct-red-test, ct-red-dsl, ct-red-external-driver, ct-green-stubs). Worth a parallel gap analysis.
