...
**Full replace**. `gh optivem atdd install` (and `upgrade`) delete every managed file and write fresh content from shop. No merge logic, no version banners, no "are you sure" prompts for files that haven't been edited (because they're not supposed to be edited).
