...
**Next move = draft a focused follow-up plan** (`/create-plan`) for the two deferred pieces, because both need a design decision before coding:
1. **`stub-fidelity-test-writer`** (Stub-only / exact-set + empty fidelity register). **First resolve the test-names plumbing**: both writers land `test-names` under one `ct-test-names` key (last-write-wins, `outputs.go:140`), and `PROBE_CONTRACT_REAL` would then run the stub-only `@Isolated` tests against the real driver. Pick: merge the two writers' names, or give the isolated stub-only suite its own probe/verify + test-names key (+ a `contract-stub-isolated` suite group). Tests author `<Sys>StubContractIsolatedTest` (mirror `ClockStubContractIsolatedTest`).
2. **Guard B** (loud "contract needed but undeclared" halt). **First categorize** which layers count as "external contract/stub" so unrelated scope-exceptions aren't mis-routed.

Then the operator items: **Step 5** (add `## External System Contract Criteria` to GitHub issue #65 in the `shop` repo) and **Step 6** (re-run the #65 rehearsal, multitier-java).

Re-enter with `/clear` then `/create-plan` for the follow-up (test-names + Guard B design), or run Step 5/6 directly.
...
- [ ] Step 3 (A, remainder) — ⏳ **Deferred → needs a focused follow-up plan.** Add the new **`stub-fidelity-test-writer`** agent (Stub-only / exact-set + empty register) + its `write-stub-fidelity-tests` process + in-callee node. **Blocked on a test-names plumbing design decision** discovered during execution: both writers land `test-names` under the single `ct-test-names` key (last-write-wins, `outputs.go:140`), and `PROBE_CONTRACT_REAL` (`suite: contract-real, test-names: ${ct-test-names}`) would then try to run the stub-only `@Isolated` tests against the real driver. Resolve first (merge the two writers' test-names, OR give the isolated stub-only suite its own probe/verify + test-names key + a `contract-stub-isolated` suite group), then implement. The stub-only fidelity tests author the `<Sys>StubContractIsolatedTest` class (mirror `ClockStubContractIsolatedTest`).
- [ ] Step 4 (B) — ⏳ **Deferred → same follow-up plan.** Make the "contract needed but undeclared" path fail loud — when the `system-implementer` scope-exception names external contract/stub files and no ESCC was declared, replace the generic `STOP_SCOPE_VIOLATION` with actionable "add a `## External System Contract Criteria` section for `<system>` and re-run" guidance. Needs a small design pass on **which layers count as "external contract/stub"** (path categorization in `validate-outputs-and-scopes`) so unrelated agents' scope-exceptions aren't mis-categorized; likely a new binding + gateway + error-end node off the `scope-exception-requested == true` branch (`process-flow.yaml:2786-2795`, `2815`).
