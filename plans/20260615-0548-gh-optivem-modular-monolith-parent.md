...
**The remaining work is DESIGN, not a mechanical edit — draft child #5 via `/create-plan`, do not look for code edits here.** As of 2026-06-15 the process follow-up (child #8) is ✅ **done and committed**: agents are a swappable `AgentSet` (`NewAgentSetFS`, injectable `fs.FS`), both swap axes have runnable proofs, the reuse path is documented in `doc.go`, and the ATDD process-definition packages (`actions`/`gates`/`verify`/`clauderun`/`configcheck`) now live under `internal/atdd/process/`. The generic engine is `internal/engine/statemachine` (`LoadBytes` only); build + full `go test ./...` green. **Children #1–#4, #6, #7 and the follow-up #8 are all done.** The single remaining child is **#5 — Scaffolding (`steps`/`templates`/`files`) + the shared `build` module (`runner`/`compiler`)**, which was never drafted. Its package *moves* are already done (`internal/scaffolding/`, `internal/build/`); what child #5 must resolve is the two still-open cross-module seams — **#3 `steps → compiler, runner`** and **#4 `preflight → runner`**: where the shared `build` module formally sits, and how Scaffolding *and* Process both depend on it without a cycle. **Next action: `/create-plan` to draft child #5.** Still un-moved at root by design (revisit if child #5's analysis calls for it): `expand`, `userstate`, `assets`, and the generic `atdd/runtime/*` packages that stayed (`agents`, `intake`, `outlog`, `override`, `trace`, `release`, `repolocator`, `testselect`, `tracker/**`, `preflight`, `driver`).
...
**Confirmed child order:** ~~Dev-workflow~~ ✅ → ~~Architecture/diagrams~~ ✅ → ~~Diagnostics~~ ✅ → ~~Config~~ ✅ → ~~Scaffolding~~ ✅ (moved) → ~~*invert seam #1*~~ ✅ → ~~Process (Child 1) first pass~~ ✅ → ~~Process follow-up (child #8)~~ ✅ (done 2026-06-15). Remaining: **child #5 — Scaffolding + shared `build` module**, to *formalize* the already-done moves and resolve seams #3 (`steps → compiler, runner`) and #4 (`preflight → runner`) — design work, draft via `/create-plan`.
