...
**Process follow-up (child #8) is now ✅ DRAFTED — execute it. Switch to `/execute-plan plans/20260615-1047-child8-process-swappable-agents-untangle.md`.** As of 2026-06-15 the engine↔definition seam is broken: the generic engine lives at `internal/engine/statemachine` (exposes only `LoadBytes`, embeds no process), and the ATDD BPMN lives at `internal/atdd/process` (`Load()` wrapping `LoadBytes`). Build + full `go test ./...` green. **All seven children's first passes are complete** and the deferred follow-up is drafted as child #8 with its decisions locked (bound `AgentSet` struct; shared chunks stay global; `doc.go` reuse path; coupling analysis via subagent). The single next executable unit is **child #8 Step 7** — introduce a bound `AgentSet` struct in `internal/atdd/runtime/agents/embed.go` (parameterise the hardcoded `agentsDir`, five funcs become methods, ATDD as `DefaultAgentSet()`), keeping `driver`/`clauderun`/`agents/registry.go` green. The parent's own mechanical work is finished; remaining decomposition lives entirely in child #8. Still un-moved at root by design pending that child: `expand`, `userstate`, `assets`, and the `atdd/runtime` internals.
...
7. **Process module: engine ↔ process definition ↔ agents** → `20260615-0549-child1-modularize-gh-optivem-engine-process.md` — ✅ **first pass done (2026-06-15)**: engine carved to `internal/engine/statemachine` (`LoadBytes` only), ATDD BPMN to `internal/atdd/process` (`Load()`); build + `go test ./...` green.
8. **Process follow-up: swappable agent set + deep-untangle** → `20260615-1047-child8-process-swappable-agents-untangle.md` — 📝 **drafted, ready to execute (2026-06-15)**: the deferred Steps 7–10 from child #7, with decisions locked (bound `AgentSet` struct; shared chunks stay global; `doc.go` reuse path; coupling analysis via subagent). Next executable unit is its Step 7.
