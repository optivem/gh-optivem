...
Step 1: in `internal/scaffolding/steps/apply_template.go`, collapse `tsMigrationsPathReplacements(arch)` (≈lines 977–1000) into an arch-independent rule `{"'../../../../db/migrations'", "'../../../db/migrations'"}`. Rewrite its doc comment and update the call-site comments (≈lines 288, 406, 484, 637) so they no longer say "5→4 up". Then do Step 2 (tests) before running `go test -p 2 ./internal/scaffolding/steps/`.

## Steps

- [ ] Step 1: `internal/scaffolding/steps/apply_template.go`:
...
- [ ] Step 2: `internal/scaffolding/steps/replacements_test.go` — replace `TestTsMigrationsPathReplacementsRewritePerArch` (≈line 1086) with a test that checks three things:
...
- [ ] Step 3: add a fail-loud drift guard in `internal/scaffolding/steps/`:
...
- [ ] Step 4: unit-test the guard with a temp dir. Cover three cases:
...
- [ ] Step 5: `go build ./...` and `go test -p 2 ./internal/scaffolding/...`. Never run unbounded `go test ./...` on Windows.
