...
7. **Bug report** — off by default. Pass `--report-bug` to auto-create a GitHub issue in `optivem/gh-optivem` with scaffolding details and the debug-branch URL on failure. Filing yourself is usually clearer and keeps scaffold config private unless you decide to share it.
