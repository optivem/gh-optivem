...
The acceptance stage workflow is `gh-acceptance-stage.yml`. It supports a `verify-level` input (defaults to `release`); scheduled runs always use `release`.
...
1. **Check for an existing run** before triggering a new one.
   ...
   - Otherwise (default), check for an existing run:
     ```bash
     gh run list --workflow gh-acceptance-stage.yml --repo optivem/gh-optivem --limit 1
     ```
   ...
   **1b. Trigger a new run:**
   ```bash
   gh workflow run gh-acceptance-stage.yml --repo optivem/gh-optivem
   ```
...
5. **If the run failed:**
   ...
   - **All fixes must be applied and verified in the clone first — never commit to shop or gh-optivem until the clone fully passes. No exceptions, even if the fix seems obvious or trivially correct. "Obvious" fixes can still break in unexpected ways — the clone verification step exists to catch that.** Apply fixes directly in the cloned scaffolded repo:
     1. Run the specific failing suite first (e.g. `gh optivem test system --suite acceptance-ui`) to quickly confirm the fix. Pass `--system path/to/systems.json` if the runner can't find it from CWD.
     2. Then run the full system test suite (`gh optivem test system` with no `--suite` filter) to catch any additional failures.
