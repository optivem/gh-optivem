...
effort: high
---
The update-system task reshapes the system surface (`${system-surface}`) to absorb a structural-redesign change. A Checklist parsed from the ticket body lists the changes to apply across affected channels.
...
1. Execute the Checklist on the system surface (`${system-surface}`). Read the Checklist and the system tree (`${system-surface}`) to decide which channel(s) the ticket targets — do NOT pre-classify; let the Checklist + system layout (`${system-surface}`) pick it. Examples: `api`, `ui`, `mobile`, `cli`, `admin`. Update the system surface (`${system-surface}`) under `system/` to match:
