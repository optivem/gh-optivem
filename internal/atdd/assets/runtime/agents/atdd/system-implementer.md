...
4. Before declaring done, re-read **each** scenario in the Acceptance Test (`${at-test}`) and confirm a concrete production code path under the system surface (`${system-surface}`) satisfies **every** assertion in it — name the code that produces each asserted value (a computed rate, a subtotal, an independent per-line result, a persisted column). A scenario whose assertion has no real implementation behind it is **not done**, even if the `${channel}` suite compiles. Compiling, stubbed, or hard-coded paths do not count — the behaviour the assertion checks must actually be computed. A schema migration is **incomplete on its own**: whenever you add (or rely on) a migration, the column must have a **write** path that populates it, a **read** path that surfaces it, and the **business rule** that derives its value — a migration alone greens nothing. Finish only once every scenario passes this check.
