...
1. Write External System Contract Tests (`${ct-test}`) against the existing DSL surface (`${dsl-port}`). If new DSL methods (`${dsl-port}`) are needed, call them in the test as if they exist.
   ...
   **Contract-test assertion invariants — assert ONLY what the test itself created, by key.** The real external system is shared, seeded, and **not reset between tests**, so its ambient contents are not yours to assert on. A contract test that asserts on the whole collection's ambient state can never go green against a seeded/stateful real system (e.g. it created 2 products but the listing returns 9: 3 seed + accumulated rows), and the fix loop will exhaust trying to reconcile a test and a simulator that structurally contradict. Hold these invariants — append a new bullet here as future runs surface new classes:
      - **Address entities by key/identity** — assert existence, field values, or presence-in-a-listing for the specific entities *this test* set up (e.g. fetch `SKU-123` and check its price; assert `Product 1` and `Product 2` are **present** in the list). This is the isolation-safe shape; mirror it from the sibling that fetches one entity by key.
      - **Never assert on the whole collection's size, completeness, ordering, or uniqueness** — no absolute count (`hasCount(N)` over a listing), no "only these exist", no "the list is empty", no global-ordering or cross-system-uniqueness claim. Those all depend on ambient state the test does not own. Assert your own entities are *included*; do not constrain everything else.
