...
// autoDefaultYAML is the smallest doc that puts an `auto-default:` block on
// a user-task. Used by Bind-time gates-registry lookup tests below.
const autoDefaultYAML = `
processes:
  sample:
    name: "Sample"
    start: CHOOSE
...
// TestBind_AutoDefault_KnownBindingResolves locks in the happy path: an
// `auto-default.binding:` that exists in the gates registry binds cleanly.
func TestBind_AutoDefault_KnownBindingResolves(t *testing.T) {
	eng, err := LoadBytes([]byte(autoDefaultYAML))
	if err != nil {
		t.Fatalf("LoadBytes: %v", err)
	}
	eng.ActionFn = func(name string) NodeFn { return func(ctx *Context) Outcome { return Outcome{} } }
	eng.AgentFn = func(name string) NodeFn { return func(ctx *Context) Outcome { return Outcome{} } }
	eng.GateFn = func(name string) NodeFn {
		if name == "refactor-type-choice" {
			return func(ctx *Context) Outcome { return Outcome{Value: "none"} }
		}
		return nil
	}
	if err := eng.Bind(); err != nil {
...
// TestBind_AutoDefault_UnknownBindingErrors covers the typo-catching path:
// `binding: refactor-type` (missing `-choice`) is rejected at Bind, before
// any run starts, with the offending binding named.
func TestBind_AutoDefault_UnknownBindingErrors(t *testing.T) {
	eng, err := LoadBytes([]byte(autoDefaultYAML))
	if err != nil {
		t.Fatalf("LoadBytes: %v", err)
	}
	eng.ActionFn = func(name string) NodeFn { return func(ctx *Context) Outcome { return Outcome{} } }
	eng.AgentFn = func(name string) NodeFn { return func(ctx *Context) Outcome { return Outcome{} } }
	eng.GateFn = func(name string) NodeFn { return nil } // every binding unknown
	bindErr := eng.Bind()
	if bindErr == nil {
		t.Fatal("Bind succeeded; want error for unknown auto-default.binding")
	}
	if !strings.Contains(bindErr.Error(), "auto-default.binding") || !strings.Contains(bindErr.Error(), "refactor-type-choice") {
		t.Errorf("error %q must name auto-default.binding + the offending binding", bindErr.Error())
	}
}
