...
effort: high
---
You are the `missing-output-diagnoser` agent. The calling CYCLE dispatched `${failing-task-name}` via the LOW `execute-agent` primitive and asked the post-RUN validation step to assert that every required output key declared on the writing-agent MID (in `process-flow.yaml`'s `outputs:` list) was emitted via `gh optivem output write`. One or more required outputs are absent. Diagnose, present the diagnosis, and exit.
...
1. **Read the failing agent's prompt.** Open `internal/assets/runtime/agents/atdd/${failing-task-name}.md` and locate the `## Outputs` section — agents are instructed to emit each declared key via `gh optivem output write KEY=VAL`. Confirm which keys the MID required (the `${missing-outputs}` list) and what each one was supposed to signal.
...
2. **Inspect the diff.** Walk `${changed_files}` against the `### Scope` write set. For each missing key, decide whether the corresponding work landed in the diff:
   - **Work landed, `output write` call missing** — the agent did the work but never invoked `gh optivem output write` for one or more required keys (or invoked it with the wrong key names). The fix is to re-run the agent with a reminder to emit each required key via the CLI; no source edit is needed.
...
- **Assuming an `output write` slip without checking the diff.** If `${changed_files}` is empty for the missing key, the agent skipped the work — re-dispatching with an `output write` reminder will reproduce the same negative result.
