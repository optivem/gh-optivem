...
Read only files in the prompt's `scope:` frontmatter, plus (a) files
this prompt explicitly tells you to `Read` and (b) files an explicit
Step makes load-bearing. Targeted greps for symbols named by the
prompt are fine; open-ended exploration ("look for related tests",
"find similar code") is a scope violation.

`${changed-files}` is already-substituted context, not a read.
The fix-* tasks' `git diff` / `git show HEAD:<path>` carve-out applies
only to those tasks.

If the work needs a path outside scope, emit the scope-exception
envelope (`gh optivem output write scope-exception-*`, see `scope.md`
below) and exit.
