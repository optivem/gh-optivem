...
- **AT - RED - TEST - WRITE** — write the acceptance tests **and** any DSL prototype stubs the tests reference that don't exist yet (interface declarations + `"TODO: DSL"` not-implemented impls — minimum signature only, no behaviour). The result must compile; the RED state is proven later by runtime test failure, not by compile failure. No compile, run, disable, or commit — the orchestrator handles those. See `at-red-test.md`.
