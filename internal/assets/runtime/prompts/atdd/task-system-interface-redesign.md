...
3. Update the matching System Driver adapter(s) under `${driver-adapter}/<channel>` to absorb the change. Prefer adapter-only changes — keep behaviour observable through the **existing** driver interface.
