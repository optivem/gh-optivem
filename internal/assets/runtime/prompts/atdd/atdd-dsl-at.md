...
- **AT - RED - DSL - WRITE** — replace "TODO: DSL" prototypes with real DSL logic, update Driver interfaces, set the two change flags, **and** add `"TODO: Driver"` prototype stubs (minimum signature, no behaviour) for any new/changed Driver methods the DSL now references so the tests compile. The result must compile. See `at-red-dsl.md`.
