...
// project package outside internal/kernel/**, which is what keeps build from
// growing a back-edge and turning the shared dependency into an import cycle.
func TestBuildImportsKernelOnly(t *testing.T) {
	...
	err := filepath.WalkDir(".", func(path string, d fs.DirEntry, err error) error {
		...
		for _, imp := range af.Imports {
			...
			if strings.HasPrefix(p, projectPrefix) && !strings.HasPrefix(p, kernelPrefix) {
				t.Errorf("%s imports %q: internal/build/** may import only internal/kernel/** "+
					"(build is a shared leaf module; importing Scaffolding/Process would create a cycle).", path, p)
			}
		}
		return nil
	})
